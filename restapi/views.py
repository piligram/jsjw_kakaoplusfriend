...
def get_seoul_weather():
    try:
        ...
        msg = "오늘 서울의 온도는 {0}도, 최저 온도는 {1}도, 최고 온도는 {2)도, 습도는 {3}%입니다. ".format(temp, min_temp, max_temp, humid)
...
def get_exchange():
    try:
        ...
        ratio = 100
        msg = "현재 환율은 {0}엔에 {1}원입니다".format(ratio, round(float(jtok) * ratio, 2))
...
def keyboard(request):
    return \
    HttpResponse(get_pretty_print(keyboard_default())
...
def message(request):
    if request.method == 'POST': 
        ...
        return \
        HttpResponse(get_pretty_print(message_default)
                  , content_type="application/json; charset=utf-8")
    else:
